...
public class ProductUpdatedEventHandlerTests
{
    ...
    {
        var productEvent = new ProductDomainEvent { ProductId = Guid.NewGuid(), Sku = "SKU", Name = "N2", PriceAmount = 2m, PriceCurrency = "USD", OccurredOn = DateTimeOffset.UtcNow };
        var payload = JsonSerializer.Serialize(productEvent, _opts);

        var wrapper = new OutboxMessageWrapper { Id = "2", EventType = "ProductUpdatedEvent", Payload = payload };
        ...
        var mockLogger = new Mock<ILogger<ProductUpdatedEventHandler>>();

        var handler = new ProductUpdatedEventHandler(_opts, mockProjection.Object, mockLogger.Object);
    ...
    {
        var wrapper = new OutboxMessageWrapper { Id = "2", EventType = "ProductUpdatedEvent", Payload = "not-json" };
        var token = CancellationToken.None;
